...
use \Exception;
use \PDO;
use Common\DbBundle\Model\App as ChildApp;
use Common\DbBundle\Model\AppQuery as ChildAppQuery;
use Common\DbBundle\Model\Map\AppTableMap;
 ...
 * @method     ChildAppQuery orderByBundleId($order = Criteria::ASC) Order by the bundle_id column
 *
 * @method     ChildAppQuery groupById() Group by the id column
 * @method     ChildAppQuery groupByName() Group by the name column
 * @method     ChildAppQuery groupByBundleId() Group by the bundle_id column
 *
 ...
 * @method     ChildApp findOneByBundleId(int $bundle_id) Return the first ChildApp filtered by the bundle_id column *
 ...
 * @method     ChildApp requireOneByBundleId(int $bundle_id) Return the first ChildApp filtered by the bundle_id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildApp[]|ObjectCollection findByBundleId(int $bundle_id) Return ChildApp objects filtered by the bundle_id column
...
abstract class AppQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \Common\DbBundle\Model\Base\AppQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\Common\\DbBundle\\Model\\App', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT `id`, `name`, `bundle_id` FROM `app` WHERE `id` = :p0';
        try {
    ...
    public function filterByBundleId($bundleId = null, $comparison = null)
    {
        if (is_array($bundleId)) {
            ...
            if (isset($bundleId['min'])) {
                $this->addUsingAlias(AppTableMap::COL_BUNDLE_ID, $bundleId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($bundleId['max'])) {
                $this->addUsingAlias(AppTableMap::COL_BUNDLE_ID, $bundleId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(AppTableMap::COL_BUNDLE_ID, $bundleId, $comparison);
    }
