...
;
	...
	function html() {
		...
		if ($this->_config ['total_record'] > $this->_config ['limit']) {
			...
			// Nút prev và first
			if ($this->_config ['current_page'] > 1) {
				$p .= '<span class="icon"><a href="' . $this->__link ( '1' ) . '">First</a></span>';
				$p .= '<span class="icon"><a href="' . $this->__link ( $this->_config ['current_page'] - 1 ) . '">Prev</a></span>';
			}
			...
			for($i = 1; $i <= $this->_config ['total_page']; $i ++) {
				...
				if ($this->_config ['current_page'] == $i) {
					$p .= '<span  class="icon">' . $i . '</span></li>';
				} else {
					$p .= '<span class="icon"><a href="' . $this->__link ( $i ) . '">' . $i . '</a></span>';
				}
			}
			
			// Nút last và next
			if ($this->_config ['current_page'] < $this->_config ['total_page']) {
				$p .= '<span class="icon"><a href="' . $this->__link ( $this->_config ['current_page'] + 1 ) . '">Next</a></span>';
				$p .= '<span class="icon"><a href="' . $this->__link ( $this->_config ['total_page'] ) . '">Last</a></span>';
			}
