...
use \Exception;
use \PDO;
use AdvertiserBundle\Model\Department as ChildDepartment;
use AdvertiserBundle\Model\DepartmentQuery as ChildDepartmentQuery;
...
abstract class Employee implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\AdvertiserBundle\\Model\\Map\\EmployeeTableMap';
    ...
    /**
     * @var        ObjectCollection|ChildWarehouse[] Collection to store aggregation of ChildWarehouse objects.
     */
    protected $collWarehouses;
    protected $collWarehousesPartial;
     ...
     * @var ObjectCollection|ChildWarehouse[]
     */
    protected $warehousesScheduledForDeletion = null;
    ...
    /**
     * Initializes internal state of AdvertiserBundle\Model\Base\Employee object.
     */
     ...
     * @return $this|\AdvertiserBundle\Model\Employee The current object (for fluent API support)
     */
    ...
    public function setUsername($v)
    {
        ...
        if ($this->username !== $v) {
            $this->username = $v;
            $this->modifiedColumns[EmployeeTableMap::COL_USERNAME] = true;
        }
    ...
    public function setPassword($v)
    {
        ...
        if ($this->password !== $v) {
            $this->password = $v;
            $this->modifiedColumns[EmployeeTableMap::COL_PASSWORD] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : EmployeeTableMap::translateFieldName('DepartmentId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->department_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : EmployeeTableMap::translateFieldName('Username', TableMap::TYPE_PHPNAME, $indexType)];
            $this->username = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : EmployeeTableMap::translateFieldName('Password', TableMap::TYPE_PHPNAME, $indexType)];
            $this->password = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : EmployeeTableMap::translateFieldName('Lastname', TableMap::TYPE_PHPNAME, $indexType)];
            $this->lastname = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : EmployeeTableMap::translateFieldName('Firstname', TableMap::TYPE_PHPNAME, $indexType)];
            $this->firstname = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : EmployeeTableMap::translateFieldName('Phone', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 8 + $startcol : EmployeeTableMap::translateFieldName('CreatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->created_at = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 9 + $startcol : EmployeeTableMap::translateFieldName('UpdatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            ...
            return $startcol + 10; // 10 = EmployeeTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\AdvertiserBundle\\Model\\Employee'), 0, $e);
        }
    }
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->warehousesScheduledForDeletion !== null) {
                if (!$this->warehousesScheduledForDeletion->isEmpty()) {
                    foreach ($this->warehousesScheduledForDeletion as $warehouse) {
                        // need to save related object because we set the relation to null
                        $warehouse->save($con);
                    }
                    $this->warehousesScheduledForDeletion = null;
                }
            }

            if ($this->collWarehouses !== null) {
                foreach ($this->collWarehouses as $referrerFK) {
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(EmployeeTableMap::COL_USERNAME)) {
            $modifiedColumns[':p' . $index++]  = '`username`';
        }
        if ($this->isColumnModified(EmployeeTableMap::COL_PASSWORD)) {
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->collWarehouses) {
                ...
                $result[$key] = $this->collWarehouses->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\AdvertiserBundle\Model\Employee
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\AdvertiserBundle\Model\Employee The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(EmployeeTableMap::COL_USERNAME)) {
            $criteria->add(EmployeeTableMap::COL_USERNAME, $this->username);
        }
        if ($this->isColumnModified(EmployeeTableMap::COL_PASSWORD)) {
            $criteria->add(EmployeeTableMap::COL_PASSWORD, $this->password);
        }
        if ($this->isColumnModified(EmployeeTableMap::COL_LASTNAME)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \AdvertiserBundle\Model\Employee (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy) {
            ...
            foreach ($this->getWarehouses() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addWarehouse($relObj->copy($deepCopy));
                }
            }
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \AdvertiserBundle\Model\Employee Clone of current object.
     * @throws PropelException
     */
    ...
    /**
     * Clears out the collWarehouses collection
     *
    ...
    {
        $this->collWarehouses = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Reset is the collWarehouses collection loaded partially.
     */
    public function resetPartialWarehouses($v = true)
    {
        $this->collWarehousesPartial = $v;
    }

    /**
     * Initializes the collWarehouses collection.
     *
     * By default this just sets the collWarehouses collection to an empty array (like clearcollWarehouses());
     ...
     * @return void
     */
    public function initWarehouses($overrideExisting = true)
    {
        if (null !== $this->collWarehouses && !$overrideExisting) {
            return;
        }

        $collectionClassName = WarehouseTableMap::getTableMap()->getCollectionClassName();

        $this->collWarehouses = new $collectionClassName;
        $this->collWarehouses->setModel('\AdvertiserBundle\Model\Warehouse');
    }

    /**
     * Gets an array of ChildWarehouse objects which contain a foreign key that references this object.
     *
     ...
     * @param      ConnectionInterface $con optional connection object
     * @return ObjectCollection|ChildWarehouse[] List of ChildWarehouse objects
     * @throws PropelException
     */
    public function getWarehouses(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collWarehousesPartial && !$this->isNew();
        if (null === $this->collWarehouses || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collWarehouses) {
                // return empty collection
                $this->initWarehouses();
            } else {
                $collWarehouses = ChildWarehouseQuery::create(null, $criteria)
                    ->filterByEmployee($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collWarehousesPartial && count($collWarehouses)) {
                        $this->initWarehouses(false);

                        foreach ($collWarehouses as $obj) {
                            if (false == $this->collWarehouses->contains($obj)) {
                                $this->collWarehouses->append($obj);
                            }
                        }

                        $this->collWarehousesPartial = true;
                    }

                    return $collWarehouses;
                }

                if ($partial && $this->collWarehouses) {
                    foreach ($this->collWarehouses as $obj) {
                        if ($obj->isNew()) {
                            $collWarehouses[] = $obj;
                        }
                    }
                ...
                $this->collWarehouses = $collWarehouses;
                $this->collWarehousesPartial = false;
            }
        }

        return $this->collWarehouses;
    }

    /**
     * Sets a collection of ChildWarehouse objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $warehouses A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildEmployee The current object (for fluent API support)
     */
    public function setWarehouses(Collection $warehouses, ConnectionInterface $con = null)
    {
        /** @var ChildWarehouse[] $warehousesToDelete */
        $warehousesToDelete = $this->getWarehouses(new Criteria(), $con)->diff($warehouses);


        $this->warehousesScheduledForDeletion = $warehousesToDelete;

        foreach ($warehousesToDelete as $warehouseRemoved) {
            $warehouseRemoved->setEmployee(null);
        }
        ...
        $this->collWarehouses = $warehouses;
        $this->collWarehousesPartial = false;
    ...
    /**
     * Returns the number of related Warehouse objects.
     *
     * @param      Criteria $criteria
     * @param      boolean $distinct
     * @param      ConnectionInterface $con
     * @return int             Count of related Warehouse objects.
     * @throws PropelException
     */
    public function countWarehouses(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collWarehousesPartial && !$this->isNew();
        if (null === $this->collWarehouses || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collWarehouses) {
            ...
            if ($partial && !$criteria) {
                return count($this->getWarehouses());
            }

            $query = ChildWarehouseQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collWarehouses);
    }

    /**
     * Method called to associate a ChildWarehouse object to this object
     * through the ChildWarehouse foreign key attribute.
     *
     * @param  ChildWarehouse $l ChildWarehouse
     * @return $this|\AdvertiserBundle\Model\Employee The current object (for fluent API support)
     */
    public function addWarehouse(ChildWarehouse $l)
    {
        if ($this->collWarehouses === null) {
            $this->initWarehouses();
            $this->collWarehousesPartial = true;
        ...
        if (!$this->collWarehouses->contains($l)) {
            $this->doAddWarehouse($l);

            if ($this->warehousesScheduledForDeletion and $this->warehousesScheduledForDeletion->contains($l)) {
                $this->warehousesScheduledForDeletion->remove($this->warehousesScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param ChildWarehouse $warehouse The ChildWarehouse object to add.
     */
    protected function doAddWarehouse(ChildWarehouse $warehouse)
    {
        $this->collWarehouses[]= $warehouse;
        $warehouse->setEmployee($this);
    }

    /**
     * @param  ChildWarehouse $warehouse The ChildWarehouse object to remove.
     * @return $this|ChildEmployee The current object (for fluent API support)
     */
    public function removeWarehouse(ChildWarehouse $warehouse)
    {
        if ($this->getWarehouses()->contains($warehouse)) {
            $pos = $this->collWarehouses->search($warehouse);
            $this->collWarehouses->remove($pos);
            if (null === $this->warehousesScheduledForDeletion) {
                $this->warehousesScheduledForDeletion = clone $this->collWarehouses;
                $this->warehousesScheduledForDeletion->clear();
            }
            $this->warehousesScheduledForDeletion[]= $warehouse;
            $warehouse->setEmployee(null);
        }
    ...
    public function setInventoryDeliveryVouchers(Collection $inventoryDeliveryVouchers, ConnectionInterface $con = null)
    {
        ...
        foreach ($inventoryDeliveryVouchersToDelete as $inventoryDeliveryVoucherRemoved) {
            $inventoryDeliveryVoucherRemoved->setEmployee(null);
        }
    ...
    protected function doAddInventoryDeliveryVoucher(ChildInventoryDeliveryVoucher $inventoryDeliveryVoucher)
    {
        ...
        $inventoryDeliveryVoucher->setEmployee($this);
    }
    ...
    public function removeInventoryDeliveryVoucher(ChildInventoryDeliveryVoucher $inventoryDeliveryVoucher)
    {
        if ($this->getInventoryDeliveryVouchers()->contains($inventoryDeliveryVoucher)) {
            ...
            $inventoryDeliveryVoucher->setEmployee(null);
        }
     ...
     * @return ObjectCollection|ChildInventoryDeliveryVoucher[] List of ChildInventoryDeliveryVoucher objects
     */
    public function getInventoryDeliveryVouchersJoinWarehouse(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildInventoryDeliveryVoucherQuery::create(null, $criteria);
    ...
    {
        if ($deep) {
            if ($this->collWarehouses) {
                foreach ($this->collWarehouses as $o) {
